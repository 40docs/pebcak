applyTo: 'lab_forticnapp_opal/**'
description: 'Agent-mode Copilot process for building FortiCNAPP Opal lab via Terraform/GitHub MCP using non-public exercise definitions'
---
...
- Operate in agent-mode only.
- Access only the `lab_forticnapp_opal` repository.
- Use `terraform-mcp` and `github-mcp` where applicable.
- Do not access or modify other repositories.
- All approval gates must be handled manually by user (no auto-approvals).
- All MCP access should be scoped to minimal permissions required for lab task execution only.
- Exercise instructions are derived solely from a provided `.md` file located at project root (`/the_how.md`).
